...
#   3. `tauri build --bundles app` → WeruBWorker.app (resources are copied in).
...
echo "==> [1/5] PyInstaller: bundling werubworker-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/werubworker-server.spec"
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/werubworker-server-$TRIPLE"
...
cp -RL "$HERE/dist/werubworker-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/werubworker-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "werubworker-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/werubworker-server"
fi
