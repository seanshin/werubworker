  ...
  if (!open) {
    return (
      <button className={"addfolder-trigger" + (compact ? " compact" : "")} onClick={() => setOpen(true)}>
        <Icon name="folderPlus" size={15} /> {t("session:addFolder.giveAccess")}
      </button>
    );
  }
    ...
    <div className="addfolder-form">
      <div className="addfolder-row">
        <input
          ...
          autoFocus
          placeholder={t("session:addFolder.choosePaste")}
          value={path}
          spellCheck={false}
        ...
        />
        <button className="btn icon-only" onClick={browse} title={t("session:addFolder.chooseLocation")} aria-label={t("session:addFolder.chooseLocation")}>
          <Icon name="folder" size={15} />
        </button>
      </div>
      <div className="addfolder-actions">
        <label className="addfolder-write" title={t("session:addFolder.writeTitle")}>
          <input type="checkbox" checked={writable} onChange={(e) => setWritable(e.target.checked)} />
          {t("session:addFolder.allowWrites")}
        </label>
