  ...
  return (
    <aside className={"right-rail" + (selected ? " artifact-mode" : "")}>
      {selected ? (
        ...
        <>
          <RailSection title={t("session:rail.progress")} open={open.progress} onToggle={() => setOpen({ ...open, progress: !open.progress })}>
            <ProgressSummary running={running} toolNames={toolNames} todo={todo} />
          </RailSection>

          {showArtifacts && (
          <RailSection
            title={`${t("session:rail.artifacts")}${artifacts.length ? ` (${artifacts.length})` : ""}`}
            open={open.artifacts}
            ...
            action={
              <>
                ...
                <button className="rail-mini-btn" onClick={(e) => { e.stopPropagation(); refreshArtifacts(); }} title={t("session:rail.refreshArtifacts")}><Icon name="refresh" size={13} /></button>
              </>
            }
          >
            {artifacts.length === 0 ? (
              <div className="rail-muted">{t("session:rail.noFiles")}</div>
            ) : (
...
function ProgressSummary({ running, toolNames, todo }: { running: boolean; toolNames: string[]; todo: TodoItem[] }) {
      ...
      <div className="rail-todo-list">
        ...
        {running && (
          <div className="rail-muted">
            {toolNames.length ? t("rail.workingWithTools", { count: toolNames.length }) : t("rail.working")}
          </div>
        )}
      </div>
    );
  ...
  if (running) {
    return (
      <div className="rail-muted">
        {toolNames.length ? t("rail.workingOnTaskWithTools", { count: toolNames.length }) : t("rail.workingOnTask")}
      </div>
    );
  }
  return (
  ...
  return (
    <div className="artifact-viewer">
      <div className="artifact-head">
        <button className="artifact-icon-btn" onClick={onBack} aria-label={t("session:rail.backToArtifacts")} title={t("common:button.back")}>
          <Icon name="arrowLeft" size={16} />
        </button>
        <div className="artifact-heading">
          <div className="artifact-title"><span>{t("session:rail.artifacts")}</span><span className="artifact-sep">/</span><span>{artifact.name}</span></div>
        ...
        <div className="rail-actions">
          {isHtml && (
            <button
              ...
              aria-label={t("session:rail.reload")}
          ...
          {isApp && (
            <button
              ...
              aria-label={t("session:rail.openInApp")}
              title={t("session:rail.openInApp")}
            >
      ...
      <div className="artifact-preview">
        {!content ? (
          <div className="rail-muted">{t("common:status.loading")}</div>
        ) : content.error ? (
          ...
          <div className="artifact-folderlist" data-testid="artifact-folder">
            ...
            {!content.entries?.length && <div className="rail-muted">{t("session:rail.folderEmpty")}</div>}
          </div>
        ) : content.kind === "office" ? (
