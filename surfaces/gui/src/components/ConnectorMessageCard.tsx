...
function relativeTime(tsSeconds: number, t: (key: string, opts?: any) => string): string {
  ...
  if (diff < 0) return t("common:misc.justNow");
  if (diff < 45_000) return t("common:misc.justNow");
  const mins = Math.round(diff / 60_000);
  if (mins < 60) return t("common:misc.mAgo", { count: mins });
      ...
      >
        ...
        {showIds ? (
          ...
          <>
            ...
            <span className="text-[11px] text-faint ml-0.5">{t("common:misc.via", { label: entry.label })}</span>
          </>
        )}
