  ...
  return (
    <div data-testid="hubspot-detail">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={t("connectors:hubspot.title")} />
        <div className="min-w-0 flex-1">
          <h2 className="text-[20px] font-semibold tracking-tight leading-tight">{t("connectors:hubspot.title")}</h2>
          <div className="text-[12.5px] text-muted flex items-center gap-1.5">
            {c.connected ? (
              <>
                <span className="w-2 h-2 rounded-full bg-ok" />
                <span data-testid="hubspot-status">
                  {t("connectors:hubspot.portalCount", { count: portals.length })}
                </span>
              </>
            ) : (
        ...
        <button className={PILL_ACCENT} data-testid="add-portal-btn" onClick={() => setAdding(true)}>
          {t("connectors:hubspot.addPortal")}
        </button>
      </div>
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("connectors:hubspot.connectPortal")}
          </div>
        </div>
      )}
      ...
      <div className={FOOT + " mt-2"}>
        {t("connectors:hubspot.hiddenFieldsNote")}
      </div>
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={t("connectors:hubspot.addAPortal")}
  ...
  return (
    <div className={ROW} data-testid={`hubspot-portal-${p.hub_id}`}>
      <span className="min-w-0 flex-1 flex items-center gap-2">
        ...
        {p.access && (
          <span className={TAG_QUIET} data-testid={`hubspot-access-tag-${p.hub_id}`}>
            {p.access === "write" ? t("connectors:hubspot.readWrite") : t("connectors:hubspot.readOnly")}
          </span>
        )}
      ...
      {!p.default && (
        ...
        >
          {t("connectors:hubspot.makeDefault")}
        </button>
      )}
      <button
        className={XBTN}
...
function PrivacyGroup({ c, onChanged }: Pick<DetailProps, "c" | "onChanged">) {
  ...
  return (
    <>
        ...
        <div className={ROW} data-testid="hubspot-hidden-fields">
          ...
          <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
            {fields.map((f) => (
              ...
              >
                {f}
                <button className={XBTN} title={t("connectors:slack.remove")} onClick={() => save(fields.filter((x) => x !== f))}>
                  ×
                </button>
              </span>
            ))}
            <input
              ...
              placeholder={t("connectors:hubspot.propertyPlaceholder")}
              value={draft}
      ...
      <div className={FOOT}>{t("connectors:hubspot.strippedNote")}</div>
    </>
  );
}
