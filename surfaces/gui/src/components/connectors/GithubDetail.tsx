...
// The GitHub detail page (github-relay-spec section 8), the Slack page's shape: one
...
function relayHealth(gh: GithubStatus | null, t: (key: string) => string): { dot: string; text: string } {
  if (!gh) return { dot: "bg-ok", text: t("connectors:shared.liveRelay") };
  if (!gh.signed_in)
  ...
  return (
    <div data-testid="github-installations">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={t("connectors:github.title")} />
        <div className="min-w-0 flex-1">
          <h2 className="text-[20px] font-semibold tracking-tight leading-tight">{t("connectors:github.title")}</h2>
              ...
              <>
                ...
                <span data-testid="github-mode-badge">
                  {relay
                    ...
                    : t("connectors:github.connectedPAT")}
                </span>
              </>
            ) : (
        ...
        {(relay || !c.connected) && (
          ...
          >
            {t("connectors:github.addInstallation")}
          </button>
        )}
      </div>
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("connectors:github.appPerAccount")}
            {cloud?.signed_in ? "" : t("connectors:github.oneClickNeedsSignIn")}
          </div>
        </div>
      )}
      ...
      {c.connected && !relay && (
        <div className={GRP} data-testid="github-manual-card">
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("connectors:github.patToolsOnly")}
          </div>
        </div>
      )}
      ...
      {relay && listening.length > 0 && (
        <>
          <div className={GRP_H}>{t("connectors:github.listening")}</div>
          <div className={GRP}>
      ...
      {c.connected && relay && (
        <div className={FOOT + " mt-2"}>{t("connectors:github.triggers")}</div>
      )}
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={t("connectors:github.addAnInstallation")}
      ...
      <div className={GRP_H + " flex items-center gap-2"}>
        <span>
          ...
          <span className="font-normal text-faint" title={`installation ${inst.installation_id}`}>
            {"· "}{inst.repo_selection === "all" ? t("connectors:github.allRepos") : t("connectors:github.selectedRepos")}
          </span>
        </span>
        {!tokenOk && (
          <span className={TAG_WARN} data-testid={`token-warn-${inst.installation_id}`}>
            {t("connectors:github.installRevoked")}
          </span>
        )}
      </div>
      <div className={GRP}>
        {empty ? (
          <div className={ROW}>
            <span className="min-w-0 flex-1 text-[12.5px] text-muted">
              {t("connectors:github.noOneAllowed")}
            </span>
...
function DisconnectBtn({ id, busy, onClick }: { id: string; busy: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={t("connectors:github.disconnectTitle")}
      onClick={onClick}
      disabled={busy}
    >
      {busy ? t("connectors:github.disconnecting") : t("connectors:github.disconnectInstallation")}
    </button>
  );
}
  ...
  return (
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">{t("connectors:shared.nobodyYetApprove")}</span>
        )}
        {allowed.map((login) => (
          <span
            key={login}
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title={t("connectors:shared.allowMentionTitle")}
      ...
      <button
        ...
        title={t("connectors:shared.allowMentionDiscard")}
        onClick={() => act("allow")}
      >
