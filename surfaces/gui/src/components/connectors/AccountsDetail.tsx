  ...
  return (
    <div data-testid="accounts-detail">
            ...
            {c.connected ? (
              <>
                ...
                <span data-testid="accounts-status">
                  {t("connectors:accounts.accountCount", { count: accounts.length })}
                </span>
              </>
            ) : (
        ...
        <button
          ...
          title={
            c.managed && !cloud?.signed_in
              ? t("connectors:accounts.signInForOneClick")
              : ""
          }
        >
          {busy ? t("connectors:accounts.checkBrowser") : t("connectors:accounts.addAccount")}
        </button>
      </div>
      ...
      {(showManual || !c.connected) && (
        <>
          <div className={GRP_H + (accounts.length ? "" : " !mt-0")}>
            {c.managed ? t("connectors:accounts.addManually") : t("connectors:accounts.addAnAccount")}
          </div>
      ...
      <div className={FOOT + " mt-2"}>
        {t("connectors:accounts.accountsSeparate")}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={ROW} data-testid={`account-${a.account_id}`}>
      ...
      {!a.default && (
        ...
        >
          {t("connectors:accounts.makeDefault")}
        </button>
      )}
      <button
        className={XBTN}
