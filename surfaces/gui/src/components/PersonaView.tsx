      ...
      {onBack && (
        <>
          <button
            ...
            onClick={onBack}
          >
            <Icon name="arrowLeft" size={15} /> {t("settings:personaView.back")}
          </button>
          <span className="text-faint">·</span>
        </>
      )}
      <span className="text-[13px] font-semibold">{t("settings:personaView.persona")}</span>
    </div>
  );
  ...
  if (error || !detail) {
    return (
      <main className="flex-1 min-w-0 flex flex-col bg-paper">
        {header}
        <div className="p-12 text-center text-faint text-[13px]">{error || t("settings:manageTabs.loading")}</div>
      </main>
    );
  }
        ...
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          <header className="flex items-start gap-3.5">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-[12px] text-muted">{detail.enabled ? t("settings:personaView.enabled") : t("settings:personaView.disabled")}</span>
              <Toggle checked={detail.enabled} onChange={toggleEnabled} title={t("settings:personaView.enablePersona")} />
            </div>
          </header>
          ...
          {detail.description && (
            <section>
              <div className={`${SEC_H} mb-1.5`}>{t("settings:personaView.about")}</div>
          ...
          {detail.tools.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-2`}>{t("settings:personaView.builtInCaps")}</div>
          ...
          {detail.recommends.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("settings:personaView.connectionsForBenefit")}</div>
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("settings:personaView.unlockWorkflow", { name: shortPersonaName(detail.name, personaId) })}
              </p>
                    ...
                    >
                      ...
                      {r.connected ? (
                        <span className="inline-flex items-center gap-1 text-[11.5px] text-ok shrink-0">
                          ...
                          {t("settings:personaView.connected")}
                        </span>
                      ) : (
                        <button
          ...
          {detail.default_connections.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("settings:personaView.newSessionsDefault")}</div>
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("settings:personaView.newSessionsDesc", { name: shortPersonaName(detail.name, personaId) })}
              </p>
                ...
                {detail.default_connections.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-[13px] font-medium">
                      ...
                      {!c.connected && (
                        <span className="text-[11px] text-faint font-normal"> · {t("settings:personaView.connectToEnable")}</span>
                      )}
                    </div>
                    <Toggle
                      checked={c.enabled}
                      ...
                      title={c.connected ? t("settings:personaView.onByDefault") : t("settings:personaView.connectFirst")}
                    />
                  </div>
                ))}
              </div>
          ...
          <section className="flex flex-wrap gap-x-8 gap-y-2 text-[12.5px]">
            {detail.recommended_models.length > 0 && (
              <div>
                <span className="text-faint">{t("settings:personaView.modelsLabel")}</span> ·{" "}
            ...
            {detail.default_permission_mode && (
              <div>
                <span className="text-faint">{t("settings:personaView.defaultMode")}</span> · {detail.default_permission_mode}
              </div>
            )}
            {detail.workspace && (
              <div>
                <span className="text-faint">{t("settings:personaView.workspace")}</span> · {detail.workspace}
              </div>
            )}
          </section>
        </div>
