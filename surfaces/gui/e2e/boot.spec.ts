// Cold-boot fixes (owner-hit 2026-07-23): the splash wears the real WeruBWorker mark
...
test("boot splash shows the WeruBWorker star, not the sparkle glyph", async ({ page }) => {
  ...
  await expect(page.getByText(/Starting WeruBWorker|Restoring your session/)).toBeVisible();
});
