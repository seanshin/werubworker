...
test("⌘B toggles the sidebar collapse", async ({ page }) => {
  await page.goto("/");
  const app = page.locator(".app");
  // Wait for the sidebar to render so keyboard-shortcut handlers are registered.
  await expect(page.locator(".sidebar")).toBeVisible();
...
test("RECENT header group/filter popover: switch grouping + see coworker filters", async ({
  ...
  // Switch to Chronological → the persona accordion collapses into a flat list (the "WeruBWorker"
