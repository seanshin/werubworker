...
def test_settings_rest_roundtrip(tmp_path, monkeypatch):
    ...
    assert (
        before["has_key"] is False and before["source"] is None and before["provider"] == "openai"
    )
...
def test_default_model_and_onboarding_persist(tmp_path, monkeypatch):
    ...
    assert (
        client.post("/v1/settings/default-model", json={"model": "gpt-4o"}).json()["model"]
        == "gpt-4o"
    )
...
def test_nav_layout_setting_roundtrips(tmp_path, monkeypatch):
    ...
    assert (
        client.post("/v1/settings/nav-layout", json={"nav_layout": "bogus"}).json()["nav_layout"]
        == "flat"
    )
...
def test_scratch_base_setting_persists_and_drives_provisioning(tmp_path, monkeypatch):
    ...
    # defaults to ~/WeruBWorker
    assert client.get("/v1/settings").json()["scratch_base"] == "~/WeruBWorker"
