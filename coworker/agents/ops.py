...
OPS_INSTRUCTIONS = (
    ...
    "- ALWAYS use server_status for CPU, memory, disk, uptime — NEVER parse shell commands like "
    "top, free, df manually. server_status returns structured JSON with cpu_percent, memory "
    "(total_gb, used_gb, available_gb, percent), disk_root, swap, load_avg, cpu_per_core, and "
    "disk_partitions. This is far more accurate and complete than shell output.\n"
    "- Use system_info for OS, architecture, CPU model, total memory/disk, uptime.\n"
    "- Use process_list for running processes (with CPU%, MEM%, pid, name, cmdline, username).\n"
    "- Use check_ports to verify if ports are open.\n"
    "- Use service_status to check systemd/launchctl service state.\n"
    "- Use disk_usage for detailed disk info on any mount point.\n"
    "- Use system_logs for reading logs (journalctl/log show).\n"
    "- Only fall back to shell commands when the monitoring tools cannot provide the needed info.\n\n"
    "## Wiki Usage\n"
