...
**대상 파일**: `surfaces/gui/src/components/`의 `LogView.tsx`, `AuditView.tsx`, `MonitoringView.tsx`
(기획 당시 `surfaces/react/src/views/`로 적었으나 실제 경로는 `surfaces/gui/src/components/`다)

| 작업 | 설명 | 결과 |
|------|------|------|
| 감사 로그 목록 | 가상 스크롤 적용 (react-window) | ✅ 150건 기준 |
| **로그 뷰어** | **기획서에 없던 최대 목록 — 아래 참조** | ✅ 2,501 → 78 노드 |
| 인시던트 목록 | 무한 스크롤 + 페이지네이션 API | ✅ limit/offset + total, "더 보기" |

##### 가장 큰 목록은 감사 로그가 아니라 로그 뷰어였다

기획서는 감사 로그 테이블을 지목했지만(150건), 실제로 가장 많은 노드를 만드는 건
`LogView.tsx`였다. **`limit=500`으로 가져와 10초마다 통째로 다시 렌더한다** — 행당 5노드씩
2,501노드가 매 갱신마다 헐리고 다시 만들어진다.
...
`react-window`는 이미 의존성에 있었고 `Sidebar.tsx`에 선례가 있어 같은 관용구를 따랐다.

**줄바꿈을 유지하기 위해 동적 행 높이를 썼다.** 로그 줄은 `whitespace-pre-wrap`으로 감싸지므로
스택 트레이스는 한 줄짜리 항목의 몇 배 높이다. 고정 행 높이를 쓰면 긴 줄이 잘리거나 짧은 줄
뒤에 빈 공간이 남는다. `useDynamicRowHeight`로 측정해서 기존 표시 동작을 그대로 뒀다.

**자동 스크롤 경로가 바뀐다.** 가상 목록은 자체 스크롤 컨테이너를 갖기 때문에 바깥 div의
`scrollTop`을 조작하던 기존 코드는 아무 일도 하지 않는다. `listRef.scrollToRow()`로 바꿨다.

행 컴포넌트는 가상/평면 양쪽이 공유한다 — 둘이 갈라지면 60건을 넘겨야만 드러나는 차이가 된다.

##### 인시던트: 성능이 아니라 도달 불가 문제였다

`list_incidents`는 원래부터 50건에서 잘렸는데 offset이 없어서 **51번째 이후 인시던트를 가져올
방법이 아예 없었다.** 화면에 안 보이는 게 아니라 API로도 못 꺼낸다. `limit`/`offset`을 추가하고
`total`·`has_more`를 함께 돌려주도록 해서, UI가 "잘렸다"는 사실을 표시하고 이어서 가져올 수 있게
했다(`이전 인시던트 N건 더 보기`). 이어붙일 때는 id로 중복을 걸러 갱신이 중간에 끼어들어도 행이
겹치지 않게 했다.

##### 테스트 환경

jsdom에는 `ResizeObserver`가 없어서 react-window가 마운트에서 터진다. `src/test-setup.ts`에
스텁을 두고 `vitest.config.ts`의 `setupFiles`로 연결했다 — 이게 없으면 테스트가 비가상 경로만
검증하게 된다.
...
| **10** | 5-1 가상 스크롤 | 대규모 로그 렌더링 | 중간 | ✅ 완료 (로그 뷰어 2,501→78 노드, 감사 로그, 인시던트 페이지네이션) |
