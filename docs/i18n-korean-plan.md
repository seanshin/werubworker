...
| # | 작업 | 상태 |
|---|------|------|
| 24 | 전체 UI 한국어 표시 스크린샷 검증 | 미착수 |
| 25 | 레이아웃 깨짐 확인 (긴 텍스트, 고정 너비) | 미착수 |
| 26 | 누락 문자열 점검 (영어 노출 탐색) | v2.3.11로 완료 — 남은 한글은 주석과 언어 전환 버튼 라벨뿐 |
| 27 | E2E 테스트 업데이트 (data-testid 전환) | 미착수 — `e2e/`가 텍스트 셀렉터 507곳, testid 683곳을 섞어 쓴다 |
| 28 | 서버 발생 문자열 검토 및 처리 | 일부 — `security_score`의 한국어 status는 v2.3.11에서 `status_code`로 분리 |
...
*작성일: 2026-08-06*
*최종 수정: 2026-08-25 (Phase 5 진행 상황 추가)*
