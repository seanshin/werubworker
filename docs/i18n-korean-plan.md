...
## 9. 진행 상황 (2026-08-26 — Phase 5까지 완료)

Phase 1~4는 v2.3.7~v2.3.10에서 마쳤다. **다만 이 기획서가 쓰인 뒤에 생긴 화면들은 범위에
없었다** — ITMS/운영 뷰(개발·토폴로지·위키 편집기·백업·보안·데이터베이스)와 그 사이드바
내비 라벨은 한국어로만 만들어졌고, v2.3.8의 "배선 완주"에도 포함되지 않았다. v2.3.11에서
이 6개 뷰 + 내비 4개를 옮기며 번역 키 134개를 추가했다.

Phase 5 항목의 현재 상태:

| # | 작업 | 상태 |
|---|------|------|
| 24 | 전체 UI 한국어 표시 스크린샷 검증 | **v2.3.13으로 완료** — 영어 누출 28곳 → 0곳 |
| 25 | 레이아웃 깨짐 확인 (긴 텍스트, 고정 너비) | **v2.3.13으로 완료** — 3개 폭 × 2개 언어 × 12개 화면, 깨짐 0건 |
| 26 | 누락 문자열 점검 (영어 노출 탐색) | v2.3.11로 완료 — 남은 한글은 주석과 언어 전환 버튼 라벨뿐 |
| 27 | E2E 테스트 업데이트 (data-testid 전환) | **v2.3.12로 완료** — 셀렉터를 바꾸는 대신 **픽스처가 로케일을 고정**하는 쪽으로 풀었다 |
| 28 | 서버 발생 문자열 검토 및 처리 | **v2.3.14로 완료** — 아래 정책으로 갈랐다 |

### 서버 문자열 정책 (항목 28)

서버가 만드는 문자열을 전부 번역하지는 않는다. **정상 동작 중 화면에 나가는 것**과
**실패했을 때만 나가는 것**은 성격이 다르다.

**정상 동작 중 나가는 것 → 구조로 보내고 문구는 클라이언트가 만든다.** 서버는 사람이 읽는
영어 한 줄을 **그대로 유지**하고(에이전트·Slack 봇·로그가 읽는다), 조각을 **덧붙여** 보낸다.
기존 소비자를 깨지 않는 가산 변경이다. 지금까지 셋:

| 서버 | 유지한 것 | 덧붙인 것 | 릴리즈 |
|---|---|---|---|
| `security_score` | `status`(`"양호"`) | `status_code`(`good`/`caution`/…) | v2.3.11 |
| `Schedule.human()` | `schedule`(`"Every day at ~5:40 PM"`) | `schedule_desc`(`{kind, hour, minute, dow}`) | v2.3.14 |
| `TaskRun.status` | — (원래부터 코드) | 클라이언트가 `session:schedule.runStatus.*`로 옮김 | v2.3.14 |

**실패했을 때만 나가는 것 → 영어로 둔다.** 백엔드의 `"error"` 문자열은 807개이고, GUI가 그걸
그대로 화면에 붙이는 지점이 87곳이다. 이들은 진단용이라 로그·이슈·에이전트가 함께 읽고,
번역하면 원문을 검색할 수 없게 된다. 번역하려면 메시지 코드 체계를 백엔드 전역에 도입해야
하는데, 얻는 것에 비해 비용이 크다. **이 판단을 뒤집으려면 코드 체계부터 설계할 것.**

**새 서버 문자열을 더할 때**: 정상 동작 중 화면에 나가는가? 그렇다면 조각을 함께 보낸다.
실패 경로에서만 나가는가? 영어로 두고 끝낸다.

### 24·25를 검증한 방법 (스크린샷만 보면 놓친다)

눈으로 보는 것만으로는 **툴팁과 접근성 라벨**을 못 잡는다. 세 갈래로 나눠 훑었다:

1. **런타임 훑기** — 한국어로 고정한 뒤 12개 화면의 DOM 텍스트와 `aria-label`/`title`/`placeholder`를
   전수로 모아, 한글이 없고 영문 단어가 있는 것만 걸러냈다. 모의 데이터·고유명사는 허용 목록으로 뺀다.
2. **정적 훑기** — 조건부로만 뜨는 UI(폴더 아티팩트 패널 등)는 런타임에 안 잡힌다. 소스에서
   `aria-label="…"` 같은 **리터럴 속성**을 직접 훑어 보완했다. 79곳 중 예시 값(`postgres`, `AKIA...`)을
   빼면 산문이 34곳이었고, 그중 **14곳은 이미 키가 있는데 배선만 안 된 것**이었다.
3. **레이아웃** — `scrollWidth > clientWidth`(가로로 잘림)와 `scrollHeight > clientHeight`(세로로 잘림)를
   잎 노드마다 확인한다. `text-overflow: ellipsis`는 설계이므로 제외해야 오탐이 사라진다.

**날짜·시각은 따로 봐야 한다.** `toLocale*`를 인자 없이 부르면 앱이 아니라 **브라우저** 언어를 따르므로,
한국어 UI가 영어 날짜를 보인다. 문자열 훑기로는 이게 "번역 누락"으로 보이지 않는다.
v2.3.13에서 `src/formatDate.ts`로 모았다.

측정 요령(주석/언어 전환 라벨을 오인하지 않기, 미참조 키 판정)은
[CHANGELOG](../CHANGELOG.md)의 v2.3.8~v2.3.11 항목에 남겼다.

---

*작성일: 2026-08-06*
*최종 수정: 2026-08-26 (Phase 5 완료)*
